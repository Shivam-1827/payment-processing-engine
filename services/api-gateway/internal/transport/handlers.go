...
func NewPaymentHandler(store idempotency.Store, producer queue.Producer, logger *slog.Logger) *PaymentHandler {
	return &PaymentHandler{idempStore: store, kafkaProducer: producer, logger: logger}
}
...
func (h *PaymentHandler) HandleCreatePayment(w http.ResponseWriter, r *http.Request) {
	...
	// ---------------------------------------------------------
	err = h.kafkaProducer.PublishIntent(ctx, idempKey, req)
	if err != nil {
		// If Kafka is down, we must delete the Redis lock so the user can retry later
		_ = h.idempStore.Delete(context.Background(), idempKey)
		h.logger.Error("kafka publish failed, released idempotency lock", "error", err)
